...
import io
import tempfile
# 🆕 الاستيرادات الإضافية للتبويب الرابع
...
# ملاحظة: هذه الدالة يتم استدعاؤها في الكود الجديد
def get_conn():
...
# ====== تبويبات ======
# 1) tab_browse 2) tab_single 3) tab_file 4) 🆕 tab_file_name_center 5) tab_count 6) tab_check 7) tab_count_custom
tab_browse, tab_single, tab_file, tab_file_name_center, tab_count, tab_check, tab_count_custom = st.tabs(
    ["📄 تصفّح السجلات", "🔍 بحث برقم", "📂 رفع ملف Excel", "📘 بحث بالاسم + المركز", "📦 عدّ البطاقات", "🧾 التحقق من المعلومات", "🧮 تحليل البيانات (COUNT)"]
)
# ----------------------------------------------------------------------------- #
# 1) 📄 تصفّح السجلات
# ----------------------------------------------------------------------------- #
...
with tab_file_name_center:
    ...
    # ✅ دالة التطبيع الأساسية للنصوص العربية (كما هي)
    ...
    # ✅ تطبيع سريع باستخدام cache (كما هي)
    @st.cache_data
    ...
    # ✅ تحميل بيانات المراكز دفعة دفعة من قاعدة البيانات (آمن) (كما هي)
    # ملاحظة: يجب أن تكون دالة 'get_conn()' معرفة مسبقًا للاتصال بقاعدة البيانات
    ...
    def load_db_for_centers(centers):
        ...
        for i in range(0, len(centers), batch_size):
            ...
            # التأكد من اختيار الأعمدة المطلوبة
            query = """
                SELECT "رقم الناخب","الاسم الثلاثي","اسم مركز الاقتراع"
                FROM "naynawa" -- 🚨 تم استخدام "naynawa" كما في الكود المرسل
                WHERE CAST("اسم مركز الاقتراع" AS TEXT) = ANY(%(centers)s)
            """
            params = {"centers": batch}

            try:
                # استخدام psycopg2.extras في حال الحاجة
    ...
    if file_nc and run_nc:
        ...
        # ---- بناء فهرس سريع للمراكز (يحتوي كل البيانات للرجوع إليها) ----
        groups = {}
        for c, sub in db_df.groupby("__norm_center"):
            groups[c] = sub.reset_index(drop=True)


        # ---- ملف مؤقت للحفظ التدريجي وتغيير رؤوس الأعمدة لمتطلباتك الجديدة ----
        temp_dir = tempfile.gettempdir()
        temp_path = os.path.join(temp_dir, "partial_results.xlsx")
        wb = load_workbook() # 🚨 يجب استخدام load_workbook() على ملف موجود أو إنشاء جديد
        # يتم إنشاء مصنف جديد فارغ
        from openpyxl import Workbook
        wb = Workbook()
        ws = wb.active
        ws.title = "نتائج مؤقتة"
        # 🆕 تم إضافة عمود "الاسم في قاعدة البيانات"
        ws.append(["الاسم في الملف", "مركز الاقتراع في الملف", "نسبة تطابق الاسم",
                    "نسبة تطابق مركز الاقتراع", "كم تكراره في قاعدة البيانات",
                    "الاسم في قاعدة البيانات", # 👈 العمود الجديد هنا
                    "رقم الناخب في قاعدة البيانات", "اسم مركز الاقتراع في قاعدة البيانات"])
        wb.save(temp_path)
        ...
        for i, row in df.iterrows():
            ...
            match_row = {
                ...
                "الاسم في قاعدة البيانات": "—", # 👈 القيمة الافتراضية للعمود الجديد
            ...
            if grp_df is not None and not grp_df.empty:
                ...
                if best_match is not None:
                    ...
                    match_row.update({
                        ...
                        "الاسم في قاعدة البيانات": best_match["الاسم الثلاثي"], # 👈 تحديث القيمة هنا
            ...
            # ✅ تحديث واجهة المستخدم والحفظ التدريجي
            ...
            if (i + 1) % 100 == 0 or i + 1 == total:
                ...
                # يجب التأكد من حفظ الترويسة في كل مرة أو استخدام openpyxl لكتابة الصفوف بشكل تدريجي
                # الطريقة الأسرع والأكثر أمانًا للحفظ المؤقت مع Pandas هي الحفظ إلى ملف مؤقت
        ...
        # لتطبيق خاصية rightToLeft على ملف الإكسل
        final_wb = load_workbook(out_file)
        final_wb.active.sheet_view.rightToLeft = True
        ...
        # قد تحتاج لإعادة حفظ النسخة المؤقتة بـ openpyxl إذا لم تكن موجودة
        # لكن بما أننا حفظناها في آخر عملية حفظ مؤقت، يمكن تحميلها مباشرة
...
# ----------------------------------------------------------------------------- #
# 7) 🧮 تحليل البيانات (COUNT) (تم إضافته لترتيب التبويبات)
# ----------------------------------------------------------------------------- #
with tab_count_custom:
    st.subheader("🧮 تحليل بيانات الناخبين حسب حقل محدد (COUNT)")
    
    st.markdown("لم يتم توفير كود هذا التبويب في السؤال الأصلي، يمكنك إضافته هنا.")