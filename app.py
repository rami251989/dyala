...
with tab_ocr:
    ...
    # ... (نفس كودك السابق للأرقام فقط + البحث)
    # (اختصرته هنا لأنه طويل ولم يتغير)

# ----------------------------------------------------------------------------- #
# 5) 📦 عدّ البطاقات
# ----------------------------------------------------------------------------- #
with tab_count:
    st.subheader("📦 عدّ البطاقات التي تحتوي رقم ناخب")

    imgs_count = st.file_uploader(
        "📤 ارفع صور البطاقات لعدّها",
        type=["jpg","jpeg","png"],
            ...
            for img in imgs_count:
                try:
                    ...
                    if texts:
                        ...
                        if found_numbers:
                            valid_cards.append(img.name)
                        else:
                            invalid_cards.append(img.name)
                except Exception as e:
                    st.warning(f"⚠️ خطأ أثناء معالجة صورة: {e}")

            st.success("✅ تم الانتهاء من العدّ")
            st.metric("عدد البطاقات المحتوية رقم ناخب", len(valid_cards))
            st.metric("عدد الصور غير المحتوية رقم ناخب", len(invalid_cards))

            if valid_cards:
                st.markdown("### 🟢 البطاقات الصحيحة:")
                st.write(valid_cards)

            if invalid_cards:
                st.markdown("### 🔴 الصور غير المعترف بها كبطاقات:")
                st.write(invalid_cards)
