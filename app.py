...
# ---------------------------------------------------------------------------
# خيار عرض الجنس (M/F أو ذكر/أنثى)
gender_display = st.radio("طريقة عرض الجنس:", ["M/F", "ذكر/أنثى"], horizontal=True)

def format_gender(df):
    if gender_display == "ذكر/أنثى":
        df["الجنس"] = df["الجنس"].map({"M": "ذكر", "F": "أنثى"})
    return df
